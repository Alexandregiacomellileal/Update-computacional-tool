...
2. **Proposed_solution.py:**
    - Module 1: A Python algorithm that automatically generates and controls massive simulations of electrical circuits using ATP (Alternative Transient Program).  This algorithm is designed to generate the $P$ sample vectors, sequentially introduce them into the clamp-on ground meter measurement circuit, and automate the steps of data insertion and extraction within the ATP software. The module creates .atp simulation files, performs simulations, and converts resulting waveforms (.pl4 files) into MATLAB files (.mat). All input-output vector pairs [**P**^m; **Z**<sub>med </sub>^m] are stored in a dedicated database. Notably, the **Z**<sub>med </sub> and **R**<sub>f</sub> database's vectors play a pivotal role as the foundational repository for the subsequent creation of a machine-learning model in Module 2. 
    It automates data insertion and removal in ATP, specifically adapted for simulations of clamp-on ground meter measurement circuits.
