...
2. **Proposed_solution.py:**
    - Python algorithm to automate data insertion and removal in the ATP (Alternative Transient Program) software, specifically adapted for simulations of clamp-on and high-frequency ground meter measurement circuits, creating .atp simulation files, performing simulations, and converting waveforms resulting (.pl4 files) in MATLAB files (.mat). All input-output pairs are stored in a dedicated database, serving as the foundational repository for training and testing the machine-learning model.
    - It also incorporates machine learning techniques to predict turbine grounding resistance based on clamp-on meter readings, utilizing data preprocessing, model training, and evaluation metrics to optimize predictive accuracy.
    - Towards the conclusion, a boxplot error graph is generated, depicting the outcomes derived from the implementation of the Clamp-on Ground Meter Method (CGM), the High-Frequency Method (HFM), and the Proposed Method in the São Bento Complex (SBC) grounding system for comparative analysis of their effects. To utilize this algorithm, users are required to download and install three essential software components: ATP, the pl42mat.exe converter, and Python. 
