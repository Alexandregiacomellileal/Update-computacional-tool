...
2. **Proposed_solution.py:**
    - Python algorithm to automate data insertion and removal in the ATP software, specifically adapted for simulations of clamp-on and high-frequency ground meter measurement circuits, working in three steps: [i] creating .atp simulation files; [ii] performing simulations; [iii] converting waveforms resulting (.pl4 files) in MATLAB files (.mat). It also incorporates machine learning techniques to predict turbine grounding resistance based on clamp-on meter readings, utilizing data preprocessing, model training, and evaluation metrics to optimize predictive accuracy. Towards the conclusion, a boxplot error graph is generated, depicting the outcomes derived from the implementation of the Clamp-on Ground Meter Method (CGM), the High-Frequency Method (HFM), and the Proposed Method in the São Bento Complex (SBC) grounding system for comparative analysis of their effects. The user must download and install 3 software: ATP (Alternative Transient Program), converter pl42mat.exe, and Python.
